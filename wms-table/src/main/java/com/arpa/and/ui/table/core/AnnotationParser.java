...
import com.arpa.and.ui.table.annotation.ColumnType;
import com.arpa.and.ui.table.annotation.Table;
import com.arpa.and.ui.table.annotation.TableColumn;
import com.arpa.and.ui.table.data.column.ArrayColumn;
import com.arpa.and.ui.table.data.column.Column;
import com.arpa.and.ui.table.data.table.PageTableData;
import com.arpa.and.ui.table.exception.TableException;
...
public class AnnotationParser <T> {
    ...
    private void getColumnAnnotation(Class clazz, String parentFieldName, List<Column> columns, Map<String, Column> parentMap, boolean isArray) {
        ...
        for (Field field : fields) {
            ...
            if (fieldAnnotation != null) {
                TableColumn tableColumn = (TableColumn) fieldAnnotation;
                ColumnType type = tableColumn.type();
                if (type == ColumnType.Own) {
                    String fieldName = parentFieldName != null ? (parentFieldName + field.getName()) : field.getName();
                    createColumn(fieldName, field, columns, parentMap, isArray, true, tableColumn);
                } else if (type == ColumnType.Child) {
                    String fieldName = (parentFieldName != null ? parentFieldName : Column.INVAL_VALUE) + field.getName() + ".";
                    ...
                    if (type == ColumnType.ArrayOwn) {
                        createColumn(fieldName, field, columns, parentMap, true, false, tableColumn);
                    } else {
     ...
     * 创建列
     */
    private void createColumn(String fieldName, Field field, List<Column> columns, Map<String, Column> parentMap, boolean isArray, boolean isThoroughArray, TableColumn tableColumn) {
        String name = tableColumn.name();
        int id = tableColumn.id();
        String parent = tableColumn.parent();
        boolean isAutoCount = tableColumn.autoCount();
        boolean isFast = tableColumn.fast();
        ...
        column.setTextAlign(tableColumn.align());
        column.setAutoMerge(tableColumn.autoMerge());
        column.setMinWidth(tableColumn.minWidth() * dp10 / 10);
        column.setMinHeight(tableColumn.minHeight() * dp10 / 10);
        column.setTitleAlign(tableColumn.titleAlign());
        column.setWidth(tableColumn.width() * dp10 / 10);
        if (tableColumn.maxMergeCount() != -1) {
            column.setMaxMergeCount(tableColumn.maxMergeCount());
        }

        column.setAutoCount(isAutoCount);
        column.setFixed(tableColumn.fixed());
