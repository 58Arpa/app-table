...
public class TableProvider <T> implements TableClickObserver {
     ...
     * 选中格子格式化
     */
    private final SelectionOperation operation;
    private final Rect clipRect;
    private final Rect tempRect; //用于存储数据
    private final PointF tipPoint = new PointF();
    private final CellInfo cellInfo = new CellInfo();
    ...
    private void drawTitle(Canvas canvas) {
        ...
        for (ColumnInfo info : columnInfoList) {
            ...
            if (info.top == 0 && info.column.isFixed()) {
                ...
                canvas.save();
                canvas.clipRect(clipRect.left, showRect.top, showRect.right, showRect.top + clipHeight);
            ...
            if (DrawUtils.isVerticalMixRect(showRect, (int) top, (int) bottom)) {
                ...
                for (int i = 0; i < columnSize; i++) {
                    ...
                    if (childColumnInfos.get(i).getTopParent().column.isFixed()) {
                        ...
                        canvas.save();
                        clipCount++;
                        canvas.clipRect(clipRect.left, showRect.bottom - countHeight, showRect.right, showRect.bottom);
                    }
    ...
    private void drawContent(Canvas canvas) {
        ...
        int columnSize = columns.size();
        int dis = config.isFixedCountRow() ? info.getCountHeight() : showRect.bottom + info.getCountHeight() - scaleRect.bottom;
